...
namespace kaizer {
    std::vector<std::string> sillyLoadMessages = {
    ...
    static std::unique_ptr<std::uniform_int_distribution<>> loadMsgDist;

    std::string_view sillyLoadMessage() {
        if(!loadMsgDist) {
            loadMsgDist = std::make_unique<std::uniform_int_distribution<>>(0, sillyLoadMessages.size() - 1);
        }
        return sillyLoadMessages[(*loadMsgDist)(randomEngine)];
    }
